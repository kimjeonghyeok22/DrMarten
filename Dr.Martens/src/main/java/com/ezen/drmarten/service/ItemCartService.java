...
public class ItemCartService {
	...
	public boolean addCartItem(CartItemList item) {
		...
		try {
			...
			//장바구니에아무물건도 없을떄
			if(itemChek == -1) this.cart.add(item); 
			//장바구니에 같은 물건이있을떄 
			...
			getCart().get(itemChek).setPrice(price+item.getPrice());
			return true;
	...
	//카트에물건 삭제할떄 
	public void name(CartItemList item) throws Exception {
		int itemChek = getCart().indexOf(item);
		...
		//만약 물건을 모두 삭제했다면
		if( total == 0)getCart().remove(itemChek); 
		//만약 삭제하는것이 장바구니에 가지고있는 물건보다 많다면 	
		if(total < 0)  throw new Exception(" 삭제 할 물건이 삭제하려는 것 보다 많습니다 "); 
		//만약 정상 삭제이라면 
